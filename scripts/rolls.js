...
function registerSettings() {
  ...
  registerSetting(constants.SETTING.ROLLS.KEY, {
    scope: "world",
    config: false,
    type: Object,
    default: {
...
function registerHooks() {
  Hooks.on("dnd5e.preRollV2", (config, dialog, message) => {
    ...
    if (roll.die !== "1d20" && dieParts) {
      ...
      config.rolls[0].options.criticalSuccess =
        dieParts.number * dieParts.faces;
...
export function isCustomRoll() {
  ...
  const customRolls = [
    ...
    rolls.skill?.die,
    rolls.tool?.die,
    ...Object.values(rolls.weaponTypes ?? {}).map(
      (weaponType) => weaponType?.die
    ),
  ];
